...
public sealed class WechatAutomation:IDisposable {
 ...
 public bool Connect(){app?.Dispose();automation?.Dispose();app=null;automation=null;var p=Process.GetProcesses().FirstOrDefault(x=>x.ProcessName.Equals("Weixin",StringComparison.OrdinalIgnoreCase)||x.ProcessName.Equals("WeChat",StringComparison.OrdinalIgnoreCase));if(p is null)return false;automation=new UIA3Automation();app=FlaUI.Core.Application.Attach(p);return true;}
