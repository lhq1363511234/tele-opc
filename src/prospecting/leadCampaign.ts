...
async function planCampaign(config: AppConfig, offer: string, icp: string, region: string, target: number) {
  ...
  const prompt = [
    ...
    '   - 名录型：榜单、排行、协会会员名单、展商名录、产业园入驻企业（一页里有很多家公司，产出最高，至少占一半）',
...
function nameKey(name: string) {
  return name
    .replace(/[（(].*?[)）]/g, '')
    .replace(/(股份)?有限(责任)?公司|集团|科技|信息|网络|数字|文化|传媒|咨询|服务|中心|工作室/g, '')
    .replace(/[\s\-·、,，.。]/g, '')
    .toLowerCase();
}
...
async function extractFromBatch(config: AppConfig, corpus: string): Promise<RawCandidate[]> {
  const prompt = [
...
async function scoreBatch(params: {
  ...
  const prompt = [
    ...
    '',
    '要求：',
    '1. 清单里每一家都要给出判断，不要漏。只有明显不可能用到我这个东西的才剔除。',
...
export async function runLeadCampaign(params: {
  ...
  for (const query of plan.queries) {
    ...
    if (hitsByHost.size >= target * 2.5) break;
  }
  ...
  const readTarget = Math.min(hits.length, Math.max(18, Math.ceil(target * 0.6)));
  await report('reading', `找到 ${hits.length} 个来源，正在读取 ${readTarget} 个页面正文…`, 0);
  const pages = await Promise.all(
    hits.slice(0, readTarget).map(async (hit) => ({ hit, body: await readPage(hit.url, 3000) }))
  );

  const sourceUrls = new Map<number, SearchHit>();
  const documents = pages.map(({ hit, body }, index) => {
    sourceUrls.set(index + 1, hit);
    return [
      `[${index + 1}] 标题：${hit.title}`,
      `网址：${hit.url}`,
      `摘要：${hit.snippet}`,
      body ? `页面正文节选：${body.replace(/\s+/g, ' ').slice(0, 1500)}` : ''
    ].filter(Boolean).join('\n');
  });

  await report('extracting', `正在从 ${documents.length} 个页面里抽取公司名…`, 0);
  const batches = chunk(documents, 6);
  const extracted = await Promise.all(batches.map((batch) => extractFromBatch(config, batch.join('\n\n'))));
  ...
  const candidates = [...deduped.values()].slice(0, Math.ceil(target * 1.6));
  ...
  const scored = (
    await Promise.all(
      scoreBatches.map((batch) => scoreBatch({ config, offer, icp, region, voiceBlock, candidates: batch, sourceUrls }))
    )
  ).flat();
  ...
  const outreachMaps = await Promise.all(
    outreachBatches.map((batch) => writeOutreach({ config, offer, voiceBlock, leads: batch }))
  );
