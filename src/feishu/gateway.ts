...
export class FeishuGateway {
  ...
  async handleEvent(event: FeishuMessageEvent) {
    if (event.chat_type !== 'p2p' || event.sender_type !== 'user' || event.message_type !== 'text') return;
    ...
    const reply = normalized.response ?? await this.brain.handleText(normalized.command ?? event.content, {
