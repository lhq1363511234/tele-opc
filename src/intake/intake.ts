...
const highRiskRules: Array<{ action: string; pattern: RegExp; reason: string }> = [
  { action: 'payment', pattern: /付款|支付|转账|退款|扣款|打款|提现/i, reason: '涉及资金动作' },
...
export function intakeMessage(text: string | undefined): IntakeResult {
  ...
  const reviewOnlyFinancialQuestion = /(?:看一下|评估|检查|分析|研究|规划|能不能|是否可以).{0,20}(?:收款|支付|账单|财务|发票|付款)/i.test(normalizedText)
    && !/(?:立即|现在|直接|马上).{0,8}(?:付款|支付|转账|退款|扣款|打款|提现)|(?:付款|支付|转账|退款|扣款|打款|提现).{0,8}(?:给|到|至|账户|银行卡|客户|供应商)/i.test(normalizedText);

  const reasons: string[] = [];
  let requiredApprovalAction: string | undefined;
  for (const rule of highRiskRules) {
    if (reviewOnlyFinancialQuestion && rule.action === 'payment') continue;
  ...
  const taskPattern = /帮我|准备|创建|整理|分析|评估|查看|看一下|检查|生成|写|起草|安排|总结|找出|寻找|挖掘|推进|规划|计划|拆解/;
